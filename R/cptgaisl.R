...
cptgaisl <- function(ObjFunc,
  ...
  if (!is.null(suggestions)) {
    ...
    if (RemainpopSize > 0) {
      ...
      fmls_init <- names(formals(popInitialize))
      init_args <- c(dots,
    ...
    init_args <- c(dots,
                   list(popSize = popSize, prange = prange, N = N,
                        ...
                        mmax = mmax, lmax = lmax))
    init_args <- .filter_args(init_args, popInitialize)
    pop <- popInitialize(popSize, prange, N, minDist, pchangepoint, mmax, lmax)
  }
  ...
  # Prepare ObjFunc routing (y only here)
  ...
  if (parallel) {
    ...
    IslandFit <- foreach::foreach(k = 1:numIslands, .combine = "cbind") %dopar% {
      apply(Island[, , k], 2, function(x) {
        ...
        call_list <- c(setNames(list(chrom), first_param),
  ...
  repeat{
    ...
    if (parallel) {
      resNewpop <- foreach::foreach(k = 1:numIslands) %dopar% {
        # Build args for NewpopulationIsland, routing + aliasing
        fmls_np <- names(formals(NewpopulationIsland))
        base_np <- c(dots,
                     list(ObjFunc = ObjFunc, prange = prange, selection = selection, crossover = crossover, mutation = mutation,
                          pop = Island[, , k], fit = IslandFit[, k],
                          minDist = minDist, lmax = lmax, mmax = mmax,
                          pcrossover = pcrossover, pmutation = pmutation, pchangepoint = pchangepoint,
                          maxgen = maxgen, N = N)
        )
        base_np <- .filter_args(base_np, NewpopulationIsland)
        do.call(NewpopulationIsland, base_np)
      }
      for (k in 1:numIslands) {
      ...
      for (k in 1:numIslands) {
        ...
        resNewpop <- do.call(NewpopulationIsland, base_np)
        tmpfit <- resNewpop[1, ]
        tmppop <- resNewpop[-1, ]

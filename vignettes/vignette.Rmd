...
This package includes the time series simulation function `ts_sim()` with the ability to introduce changepoint effects. The time series $Z_t,\ t=1,\ldots,T_s$ could be simulated from a class of time series models,
...
including the intercept term with $\beta_{0}=0.5$ and two changepoints at $\tau_{1}=50$ and $\tau_{2}=150$ with the mean changing effects of $\Delta_{1}=2$ and $\Delta_{2}=-2$. The users can also include additional covariates into matrix (`XMatT` below) for other possible model dynamics, such as trend and seasonality.
...
The simulated time series could be plotted in the figure below. The vertical dashed blue lines indicate the true changepoint location and the horizontal red dashed segments represent the time series segment sample mean. We can clearly observe the mean changing effects from the introduced changepoints $\tau_{1}=50$ and $\tau_{2}=150$. The **changepointGA** package will be used to detect these two changepoints.
...
Considering the model used in the data simulation, we could use the R function, `arima_bic_order_pq()`, included in the **changepointGA** package. This function inputs include the 
...
The structured and consensus-guided island model GA can be implemented using
the `cptgascisl()` function. It retains the basic island model structure of
`cptgaisl()` while introducing structured birth-death-relocate mutation,
cross-island consensus guidance, consensus-guided mutation candidate selection,
and objective-based local refinement.

The default mutation operator is `mutation_birth_death_relocate()`, which
performs one feasible birth, death, or relocation operation on the changepoint
configuration. When consensus guidance is enabled, information from the best
solutions across islands is used to guide mutation toward changepoint locations
with stronger cross-island support.

The strength of consensus guidance can be controlled using
`consensus_lambda`, while `consensus_radius` determines the neighborhood used
to construct the consensus score. The argument `consensus_candidates`
specifies the number of mutation proposals considered during consensus-guided
candidate selection.

Objective refinement can be controlled using `local_refine`. The available
options are `"none"`, `"final"`, and `"periodic"`. Under periodic refinement,
the best solution from each island is locally refined every `local_every`
migration cycles.

```{r}
tim5 <- Sys.time()
resSCIslandchangepointGA <- suppressWarnings(cptgascisl(
  ObjFunc = arima_bic_order_pq,
  N = N,
  prange = prange,
  popSize = 160,
...
tim6 <- Sys.time()
tim6 - tim5
print(resSCIslandchangepointGA)
summary(resSCIslandchangepointGA)
plot(resSCIslandchangepointGA, data = myts)
```

## Parallel computing

The default behavior of both `cptga()`, `cptgaisl()`, and `cptgascisl()` is to evaluate each candidate changepoint configuration sequentially, which can be computationally expensive, especially during the initial population fitness evaluation. To address this, users may enable parallel computing to significantly speed up the evaluation process.
...
The SC-IMGA can be run in parallel in the same way:

```{r}
tim7 <- Sys.time()
resSCIslandchangepointGA_parallel <- suppressWarnings(cptgascisl(
  ObjFunc = arima_bic_order_pq,
  N = N,
  prange = prange,
  popSize = 160,
...
tim8 <- Sys.time()
tim8 - tim7
summary(resSCIslandchangepointGA_parallel)
plot(resSCIslandchangepointGA_parallel, data = myts)
```
...
Li, M., & Lu, Q. *changepointGA: An R Package for Fast Changepoint Detection
via Genetic Algorithms*. Accepted for publication in *The R Journal*.

Li, M. (2026). *Structured and Consensus-Guided Island Model Genetic Algorithm
for Multiple Changepoint Detection*. Manuscript submitted for publication.

...
```{r}
ARIMA.BIC.Order(chromosome=c(2, 1, 1, 125, 250, 501), plen=2, XMat=XMatT, Xt=myts)
```
...
```{r}
pop = matrix(0, nrow=2 + N/2 - 1, ncol=40)
# put the two desired candidate solutions into pop matrix
pop[1:6,1] = c(2, 1, 1, 125, 250, N+1)
pop[1:7,2] = c(3, 1, 1,  75, 125, 250, N+1)

# fill the remain 38 individuals slot from default generation
tmppop = random_population(popsize=40, prange=p.range, N=N, minDist=1, Pb=10/N, 
                           mmax=N/2 - 1, 2 + N/2 - 1)
pop[,3:40] = tmppop[,3:40]
pop[1:10, 1:10]
...
```{r}
res.changepointGA = GA(ObjFunc=ARIMA.BIC.Order, N=N, GA_param, GA_operators = operators.self,
...
tim1 = Sys.time()
res.Island.changepointGA = IslandGA(ObjFunc=ARIMA.BIC.Order, N=N, IslandGA_param, 
                                    p.range=p.range, XMat=XMatEst, Xt=myts)
tim2 = Sys.time()
tim2 - tim1
...
Comparing to island GA without applying the parallel computing technique, the computation time here largely decreased. 
